...
#y<-model_mat[,ncol(model_mat)]
#model_mat<-model_mat[,-ncol(model_mat)]

#----------- Regression Corfficient Beta Cap -----------------

#beta_cap<-solve((t(model_mat)%*%model_mat))%*%t(model_mat)%*%y
#mat_obj$print()

#----------- The fitted values fitted_val -----------------

#fitted_val<-model_mat%*%beta_cap
...
variance_reg_coef<- diag(c(residual_variance)*(solve(t(model_mat)%*%model_mat)))
#variance_reg_coef<- as.numeric(residual_variance)*(solve(t(model_mat)%*%model_mat))
